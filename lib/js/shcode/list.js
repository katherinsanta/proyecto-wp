...
/*	jQuery('#unordered_list').change(function() {
		jQuery('#unordered_list option').each(function(index) {   
			jQuery('#unordered_sample ul li i').removeClass('icon-'+icon_class);
		});
		
		jQuery('#unordered_sample ul li i').addClass( 'icon-'+icon_class );
	});*/
	jQuery('#unordered_list').change(function() {
		var icon_class;
		if (jQuery('#unordered_list').val() == "star") {
			icon_class = 'fav';
		}else if(jQuery('#unordered_list').val() == "cancel") {
			icon_class = 'close';
		}else if(jQuery('#unordered_list').val() == "arrow") {
			icon_class = 'list-arrow';
		}else if(jQuery('#unordered_list').val() == "bullet") {
			icon_class = 'circle';
		}else if(jQuery('#unordered_list').val() == "tick") {
			icon_class = 'tick';
		}	
		jQuery('#unordered_list option').each(function(index) {  
		 
			var icon_class;
			if (jQuery(this).val() == "star") {
				icon_class = 'fav';
			}else if(jQuery(this).val() == "cancel") {
				icon_class = 'close';
			}else if(jQuery(this).val() == "arrow") {
				icon_class = 'list-arrow';
			}else if(jQuery(this).val() == "bullet") {
				icon_class = 'circle';
			}else if(jQuery(this).val() == "tick") {
				icon_class = 'tick';
			}	
...
function insertList(){
	...
	if(jQuery('#list_type').val() == 'ordered_list'){
		...
		list_shcode = '[unordered_list style="'+jQuery('#unordered_list').val()+'"] <ul> <li><i class="icon-'+icon_class+'"></i>Here goes the list item</li> </ul>	[/unordered_list]'; 
	}
