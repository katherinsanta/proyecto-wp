...
function paste1(contentID, element_id){
    ...
    /*hide slideshow list if "enable slideshow" is set "no" */
    jQuery('.enable_slideshow label').each(function() {
        jQuery(this).click(function() {
          if(jQuery(this).find('input').val() == 'no'){
            jQuery('.slideshow_options').addClass('hidden');
          }else{
            jQuery('.slideshow_options').removeClass('hidden');
          };
        });
        if(jQuery(this).find('input:checked').val() == 'no') {
            jQuery('.slideshow_options').addClass('hidden');
        }else{
            jQuery('.slideshow_options').removeClass('hidden');
        };
    ...
    /* hide "Slide speed" if autoplay is set "no" */
    jQuery('.slideshow_autoplay label').each(function() {
        jQuery(this).click(function() {
          if(jQuery(this).find('input').val() == 'no'){
            jQuery('.slide_speed').addClass('hidden');
          }else{
            jQuery('.slide_speed').removeClass('hidden');
          };
        });
        if(jQuery(this).find('input:checked').val() == 'no') {
            jQuery('.slide_speed').addClass('hidden');
...
( function( $ ){  
    ...
    $( function(){
        ...
        $( '.layout-builder').on( 'click', '.element-container.editing .element-type label input.gallery-type', function( event ){
            var $element = $( event.currentTarget).parents( '.element-container' );
            insert_select_box( '.galleries', $element );
        });
        ...
        $( '.layout-builder').on( 'click', '.element-container.editing .element-type label input.gallery-post-type', function( event ){
            var $element = $( event.currentTarget).parents( '.element-container' );
            insert_select_box( '.galeries', $element );
        });
        ...
        $( '.admin-menu .add-new').click( function(){
            ...
            if (not_updated) {
                ...
                /*hide slideshow list if "enable slideshow" is set "no" */
                $('.enable_slideshow label').each(function() {
                    $(this).click(function() {
                      if($(this).find('input').val() == 'no'){
                        $('.slideshow_options').addClass('hidden');
                      }else{
                        $('.slideshow_options').removeClass('hidden');
                      };
                    });
                ...
                /* hide "Slide speed" if autoplay is set "no" */
                $('.slideshow_autoplay label').each(function() {
                    $(this).click(function() {
                      if($(this).find('input').val() == 'no'){
                        $('.slide_speed').addClass('hidden');
                      }else{
                        $('.slide_speed').removeClass('hidden');
                      };
                    });
                    if($(this).find('input:checked').val() == 'no') {
                        $('.slide_speed').addClass('hidden');
        ...
        $( '.layout-builder' ).on( 'click', '.edit-element.edit-row', function( event ){ /*show 'edit row' Box*/
            ...
            $( this).parents( 'div.row-element').children( '.element-container').find('.my-color-field').wpColorPicker();
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.options-behavior', function( event ){
            ...
            var arr = ['empty', 'delimiter','textelement','widget_zone','post', 'page', 'gallery','teams','banners','testimonials','woo_shop'];
            
            $( event.currentTarget).toggle( !( jQuery.inArray( element_type, arr) > -1 ) );
        });

        $( '.layout-builder').on( 'show-hide', '.options-orderby, .options-order', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();

            /*add values for which to hide Order/order by option in the following array*/
            //var arr = ['best_sellers_products','woo_shop'];
            var arr = ['empty', 'delimiter','textelement','widget_zone','post', 'page', 'gallery','teams','banners','testimonials','woo_shop','best_sellers_products'];
        ...
        $( '.layout-builder').on( 'show-hide', '.options-show-excerpt, .options-show-meta, .options-content-position', function( event ){
            ...
            var arr = ['empty', 'delimiter','textelement','widget_zone','post', 'page', 'gallery','teams','banners','testimonials'];
        ...
        $( '.layout-builder').on( 'show-hide', '.masonry-options', function(event){
            ...
            $( event.currentTarget).toggle( ( view == 'grid_view' || view == 'grid_view_thumbnails') && element_type != 'empty' && element_type != 'delimiter' && element_type != 'textelement' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'page' && element_type != 'teams' && element_type != 'gallery' );
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.gutter-options', function(event){
            ...
            $( event.currentTarget).toggle( (view == 'grid_view_thumbnails') && element_type != 'empty' && element_type != 'delimiter' && element_type != 'textelement' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'page' && element_type != 'teams' && element_type != 'gallery' );
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.options-behavior', function( event ){
            ...
            if( 'featured' == $element_container.find( '.element-type input:checked').val() || 'latest' == $element_container.find( '.element-type input:checked').val() || 
                'latest_galleries' == $element_container.find( '.element-type input:checked').val() || 'latest_products' == $element_container.find( '.element-type input:checked').val() ||
                'featured_products' == $element_container.find( '.element-type input:checked').val() ||
                'on_sale_products' == $element_container.find( '.element-type input:checked').val() ||
                'best_sellers_products' == $element_container.find( '.element-type input:checked').val() ||
                'yes' == $element_container.find( '.masonry-options label input:checked').val() ){
...
/*            if($element_container.find( '.element-view-type input:checked').val() == 'news_view'){
            ...
            if( 'yes' == $element_container.find( '.masonry-options label input:checked').val() || 'list_view' == $element_container.find( '.element-view-type input:checked').val() 
                || 'mosaic_view' == $element_container.find( '.element-view-type input:checked').val() 
               /* || 'latest' == $element_container.find( '.element-type input:checked').val() 
                || 'latest_galleries' == $element_container.find( '.element-type input:checked').val()*/
                || 'woo_shop' == $element_container.find( '.element-type input:checked').val()  ){
        ...
        $( '.layout-builder').on( 'show-hide', '.options-view-type', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            if( 'latest' == element_type || 'featured' == element_type || 'latest_galleries' == element_type || 'latest_products' == element_type 
                || 'featured_products' == element_type || 'on_sale_products' == element_type || 'best_sellers_products' == element_type || 'woo_shop' == element_type) {
                clear_box( '*', $( event.currentTarget).parents( '.element-container' ) );
            }
            $( event.currentTarget).toggle( !( element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'team' || element_type == 'gallery'  ) );
        });

        $( '.layout-builder').on( 'show-hide', '.options-list-view-excerpt', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( ( element_type != 'empty' && element_type != 'delimiter' && element_type != 'textelement' && element_type != 'latest_products'  && element_type != 'featured_products' && element_type != 'on_sale_products' && element_type != 'woo_shop' && element_type != 'best_sellers_products' && element_type != 'testimonials' && element_type != 'banners' && element_type != 'teams' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'page' && element_type != 'gallery'  && element_type != 'box' && element_type != 'team' && ( $element_container.find( '.element-view-type input:checked' ).val() == 'list_view' ) ) );
        });

        $( '.layout-builder').on( 'show-hide', '.options-list-thumb-size, .options-list-title-position', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( !( element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'gallery' || element_type == 'team' || ($element_container.find( '.element-view-type input:checked' ).val() != 'list_view' || $element_container.find('.enb-list-thumbs-container input:checked').val() == 'yes' ) ) );
        });

        $( '.layout-builder').on( 'show-hide', '.hide_excerpt-options', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( !( element_type == 'textelement' || element_type == 'testimonials' || element_type == 'banners' || element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'gallery' || element_type == 'team' || ($element_container.find( '.element-view-type input:checked' ).val() != 'list_view' || $element_container.find( '.option-list-thumb-size input:checked' ).val() != 'no_thumb' || $element_container.find('.enb-list-thumbs-container input:checked').val() == 'yes' ) ) );
        });


    /*$( '.layout-builder').on( 'show-hide', '.row-element', function( event ){
        $(event).hide();
    }*/
        ...
        $( '.layout-builder').on( 'show-hide', '.options-columns', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val(); 
            $element_container.find( '.options-numberposts .hint').add( event.currentTarget).toggle( !( element_type == 'empty' || element_type == 'delimiter' || element_type == 'widget_zone' || element_type == 'textelement' || element_type == 'post' || element_type == 'page' || element_type == 'gallery' ||  ($element_container.find( '.element-view-type input:checked' ).val() == 'list_view' && !(element_type == 'teams') ) || ($element_container.find( '.element-view-type input:checked' ).val() == 'mosaic_view' && !(element_type == 'teams') ) || element_type == 'banners' || element_type == 'testimonials' ) );
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.option-numberposts', function( event ){
            ...
            var arr = ['widget_zone','post','page', 'gallery', 'banners','logo', 'searchbar','login','testimonials','copyright', 'breadcrumbs','textelement','socialicons', 'delimiter', 'empty', 'woo_shop'];
        ...
        $( '.layout-builder').on( 'show-hide', '.options-shop-number-post-hint', function( event ){
            
            /*add values for which to Show 'options-shop-number-post-hint' in the following array*/
            var arr = ['woo_shop'];
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( jQuery.inArray( element_type, arr) > -1 );
        ...
        $( '.layout-builder').on( 'show-hide', '.text_align_options', function( event ){
            ...
            var arr = ['menu', 'top_menu', 'socialicons', 'textelement', 'searchbar', 'logo', 'copyright', 'menu_and_logo', 'dynamicart', 'login', ,'widget_zone'];
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( jQuery.inArray( element_type, arr) > -1 );
        });

        $( '.layout-builder').on( 'show-hide', '.text_vertical_align', function( event ){
            
            /*add values for which to Show 'text_align_options' in the following array*/
            var arr = ['menu','socialicons', 'textelement', 'searchbar', 'copyright'];
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( jQuery.inArray( element_type, arr) > -1 );
        });

        $( '.layout-builder').on( 'show-hide', '.testimonial_options', function( event ){
            
            /*add values for which to Show 'testimonial_options' in the following array*/
            var arr = ['testimonials'];
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( jQuery.inArray( element_type, arr) > -1 );
        });
        ...
        $( '.layout-builder').on( 'show-hide', '.menu_options', function( event ){
            ...
            var arr = ['menu','top_menu', 'menu_and_logo'];
        ...
        $( '.layout-builder').on( 'show-hide', '.option-delimiter_hint', function( event ){
            ...
            if(jQuery.inArray( delimiter_type, delimiter_arr) > -1){
                ...
                $( this).find('.my-color-field').wpColorPicker();
            }
        ...
        $( '.layout-builder').on( 'show-hide', '.options-view-type', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( element_type != 'empty' && element_type != 'delimiter' && element_type != 'textelement' && element_type != 'teams' && element_type != 'banners' && element_type != 'testimonials' && element_type != 'widget_zone' && element_type != 'post' && element_type != 'page' && element_type != 'gallery'  && element_type != 'team' );
        });

        $( '.layout-builder').on( 'show-hide', '.option-list-thumb-size, .option-list-title-position', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( !( element_type == 'empty' || element_type == 'delimiter' || element_type == 'textelement' || element_type == 'woo_shop' || element_type == 'latest_products' || element_type == 'featured_products' || element_type == 'on_sale_products' || element_type == 'best_sellers_products' || element_type == 'testimonials' || element_type == 'banners' || element_type == 'teams' || element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'gallery' || element_type == 'team' || ($element_container.find( '.element-view-type input:checked' ).val() != 'list_view' || $element_container.find('.enb-list-thumbs-container input:checked').val() == 'yes' ) ) );
        });

        $( '.layout-builder').on( 'show-hide', '.enb-list-thumbs-container', function( event ){
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( !( element_type == 'empty' || element_type == 'delimiter' || element_type == 'textelement' || element_type == 'testimonials' || element_type == 'banners' || element_type == 'teams' || element_type == 'widget_zone' || element_type == 'post' || element_type == 'page' || element_type == 'gallery' || element_type == 'team' || ( $element_container.find( '.element-view-type input:checked' ).val() != 'list_view' && $element_container.find( '.element-view-type input:checked' ).val() != 'mosaic_view') ) );
        });

        $( '.layout-builder').on( 'show-hide', '.login_options', function( event ){
            
            /*add values for which to Show 'login_options' in the following array*/
            var arr = ['login'];
            var $element_container = $( event.currentTarget).parents( '.element-container' );
            var element_type = $element_container.find( '.element-type input:checked').val();
            $( event.currentTarget).toggle( jQuery.inArray( element_type, arr) > -1 );
        ...
        /*add class selected for checked input slideshow */
        
        $( '.enable_slideshow' ).find('input:checked').parent('label').addClass( 'selected' );
        $( '.slideshow_list' ).find('input:checked').parent('label').addClass( 'selected' );
        $( '.slideshow_type' ).find('input:checked').parent('label').addClass( 'selected' );
        $( '.slideshow_autoplay' ).find('input:checked').parent('label').addClass( 'selected' );
        ...
        /* for product related elements we want to hide the thumb view 
            and when list view is selected we hide the option for full width
        */
        $( '.layout-builder').on( 'show-hide', '.options-view-type', function( event ){ 
            var $element_container = $( event.currentTarget).parents( '.element-container' );

            /*add values for which to hide 'thumbnail_view' in the following array*/
            //['empty', 'delimiter','textelement','widget_zone','post', 'page', 'boxes', 'teams','banners','testimonials','woo_shop'];
            var arr = ['latest_products','featured_products','on_sale_products','woo_shop','best_sellers_products', 'empty', 'delimiter','textelement','widget_zone','post', 'page', 'boxes', 'teams','banners','testimonials'];
            var element_type = $element_container.find( '.element-type input:checked').val();


            if( jQuery.inArray( element_type, arr) > -1 ){

                /*hide thumb view*/
                $element_container.find( '.thumbnail_view, .mosaic_view').parent().hide();
                if( $element_container.find( '.thumbnail_view:checked, .mosaic_view:checked').length ){
                    $element_container.find( 'input.thumbnail_view, input.mosaic_view').prop( 'checked', false).parent().removeClass( 'selected' );
                    $element_container.find( '.element-view-type input').first().prop( 'checked', true );
                    $element_container.find( '.element-view-type input').first().parent().addClass( 'selected' );
                }

                /*option-list-thumb-size*/
                $element_container.find( '.option-list-thumb-size').hide();
                
            }else{

                /*show back thumb view*/
                $element_container.find( '.thumbnail_view, .mosaic_view').parent().show();
                $element_container.find( '.element-view-type').find( 'input').attr( 'disabled', false );

                /*option-list-thumb-size*/
               // $element_container.find( '.option-list-thumb-size').show();
            }
        });
