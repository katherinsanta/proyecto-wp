<?php
    class options{
        ...
        static function CallMenu( $name ) {
            ...
            if( !isset( $keys[ 2 ] ) ){
                $menu_arr = array_keys( self::$menu[ $menu ][ $submenu ][ 'contains' ] );
                $keys[ 2 ] = array_shift( $menu_arr );
            }
            list( ,, $tab ) = $keys;
            ...
            if( isset( self::$fields[ $group ] ) ){
                foreach( self::$fields[ $group ] as $side => $field ){
                    ...
                    if( isset( $field['type'] ) ){
                        ...
                        /*for the type 'upload' we will get the value for the id if it exists*/
                        if( isset( $type[1] ) && $type[1] == 'upload' ){
                            /*this is used for the hidden input value that is generated after and upload element*/
