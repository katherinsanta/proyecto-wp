<?php
    class LBSidebarResizer{
        ...
        function render_frontend( $callback = false ){
            ...
            if(!is_element_used($current_template -> _rows, 'gallery')){  /*if there is no gallery selected in the current template*/
            ...
            }else{ /*if in one of the rows user selected a gallery, then only that gallery will be siplayed*/
                $elem_info = get_element_info($current_template -> _rows, 'gallery');
                
                if(isset($elem_info['galleryID'])){
                    global $current_gallery;
                    $current_gallery = $elem_info['galleryID'];

                    get_template_part( 'single-gallery-content' );    
                }
