<?php
    class LBTemplateBuilder extends  LBRenderable{
        ...
        function __construct(){
            ...
            $this -> scriptdata[ 'translations' ][ 'gallery' ] = __( 'gallery' , 'cosmotheme' );
            $this -> scriptdata[ 'translations' ][ 'galleries' ] = __( 'galleries' , 'cosmotheme' );
        ...
        function list_galeries(){
            $galleries = get_posts( array( 'numberposts' => -1, 'post_type' => 'gallery' ) );
            $this -> list_singulars( $galleries, __( 'gallery', 'cosmotheme' ), __( 'posts', 'cosmotheme' ), 'galleryID' );
        }

        function list_slideshow_singulars( $singulars, $name_sg, $name_pl, $inputName, $name, $checked ){
            if( !is_wp_error( $singulars ) && is_array( $singulars ) && count( $singulars ) ){
                echo '<p>' . sprintf(__( 'Here are the latest %s.', 'cosmotheme' ), $name_pl) . '</p>';
                foreach( $singulars as $key => $singular ){
                    ?>
                <label class="choose-one" data-id="<?php echo $singular -> ID;?>">
                    <input type="radio" name="<?php echo $name;?>[<?php echo $inputName;?>]" value="<?php echo $singular -> ID;?>" <?php checked( $checked, $singular -> ID );?>>
                    <?php echo $singular -> post_title;?>
                </label>
                <?php
                }
        ...
        function list_slideshows($name, $checked){
            $slideshows = get_posts( array( 'numberposts' => -1, 'post_type' => 'slideshow' ) );
            $this -> list_slideshow_singulars( $slideshows, __( 'slideshow', 'cosmotheme' ), __( 'slideshows', 'cosmotheme' ), 'slideshowID', $name, $checked );
        }
        ...
        function list_galleries(){
            $galleries = get_terms( 'gallery-category', array( 'hide_empty' => false ) );
            $this -> list_terms( $galleries, __( 'gallery', 'cosmotheme' ), __( 'galleries', 'cosmotheme' ), 'galleries' );
        }
        
        function list_teams(){
            $teams = get_terms( 'team-group', array( 'hide_empty' => false, 'numberposts' => -1 ) );
            $this -> list_terms( $teams, __( 'team', 'cosmotheme' ), __( 'teams', 'cosmotheme' ), 'teams' );
        }

        function list_banners(){
            $banners = get_posts( array( 'numberposts' => -1, 'post_type' => 'banner' ) );
            $this -> choose_many_singulars( $banners, __( 'banner', 'cosmotheme' ), __( 'banners', 'cosmotheme' ), 'banners' );
        }

        function list_testimonials(){
            $testimonials = get_posts( array( 'numberposts' => -1, 'post_type' => 'testimonial' ) );
