...
class LBFooterElement extends LBElement{
    ...
    function render_frontend_menu(){ 
        echo menu( 'footer_menu' , array(
            'container'       => 'nav',
            'container_class' => 'footer-menu cosmo-menu align-'. $this -> vertical_align,  
    ...
    function render_frontend_copyright(){
        ?>
            <p class="copyright align-<?php echo $this -> vertical_align ?>"><?php echo str_replace('%year%',date('Y') , options::get_value('general' , 'copy_right') ); ?></p>
        <?php
    }

    function render_frontend_socialicons(){
        $social_icons = new LBHeaderElement($this);
